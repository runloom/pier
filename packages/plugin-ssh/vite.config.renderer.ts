...
/**
 * SSH plugin renderer entry build. Produces `dist/renderer.js` — a
...
export default defineConfig({
  ...
  build: {
    ...
    outDir: "dist",
    rollupOptions: {
      // The react / jsx-runtime shim subpaths must be bundled inline — the
      // browser can't resolve bare `@pier/plugin-api/react` specifiers via
      // `pier-plugin://`, and the shims only read from
      // `globalThis.__PIER_PLUGIN_SHARED__`, so bundling adds no second React.
