...
export function createCodexAccountsStateStore(
  ...
  async function quarantineCorruptFile(
    ...
    logger?.warn(
      "[pier.codex] state file is corrupt, moving aside and starting fresh",
      {
  ...
  function repairState(loaded: CodexAccountsFileState): CodexAccountsFileState {
    let repaired = loaded;
    ...
    if (deduped.length !== repaired.accounts.length) {
      logger?.warn("[pier.codex] removed duplicate account ids from state");
    ...
    if (
      ...
      logger?.warn(
        "[pier.codex] active account missing from metadata, clearing selection",
