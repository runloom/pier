  ...
  return (
    <TooltipProvider delayDuration={200}>
      {showSyncPeers ? (
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              aria-label={t(
                "pier.claude.accounts.settings.syncPeers",
                "Sync to other tools"
              )}
              onClick={onSyncPeers}
              size="icon-sm"
      ...
      <Tooltip>
        ...
        <TooltipContent data-pier-claude-scope="">
          {t("pier.claude.accounts.settings.refreshUsage", "Refresh usage")}
        </TooltipContent>
      </Tooltip>
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            aria-label={`${t("pier.claude.accounts.settings.remove", "Remove")}: ${activeLabel}`}
