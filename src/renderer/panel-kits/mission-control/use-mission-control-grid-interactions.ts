...
import {
  deriveOrderedMissionControlLayout,
  missionControlLayoutRows,
  moveMissionControlEntry,
  resolveMissionControlInsertionIndex,
} from "./mission-control-ordered-layout.ts";
import {
  alignTransientGridLayout,
  missionControlPreviewOffset,
...
interface GridInteractionsOptions {
  ...
  onResize(instanceId: string, size: MissionControlGridSize): void;
  trailingEntry: MissionControlPanelWidgetEntry;
...
export function useMissionControlGridInteractions({
  cols,
  getSizeDeclaration,
  onReorder,
  onResize,
  ...
  const renderedLayout = useMemo(
    () =>
      markTrailingStatic(
        deriveOrderedMissionControlLayout([...renderedWidgets, trailingEntry], {
          cols,
          getSizeDeclaration,
        }),
        trailingEntry.id,
  ...
  const dragPreviewOffsets = useMemo(() => {
    ...
    for (const baseItem of renderedLayout) {
      ...
      const offset = missionControlPreviewOffset(baseItem, previewItem, {
        cols,
        viewportWidth,
      });
  ...
  const dragPreviewHeight = dragPreviewLayout
    ? gridRowsHeight(
        Math.max(
          missionControlLayoutRows(renderedLayout),
          missionControlLayoutRows(dragPreviewLayout)
        )
      )
    : null;
  const resizePreview =
    ...
    (_layout, _oldItem, activeItem) => {
      ...
      setLayoutPreview((current) => {
        ...
        return {
          ...
          source: widgets,
          targetIndex,
          widgets: moveMissionControlEntry(widgets, activeItem.i, targetIndex),
        };
      });
    },
  ...
  const handleDragStop = useCallback<EventCallback>(
    (transientLayout, _oldItem, activeItem) => {
      if (activeItem) {
        const targetIndex = resolveInsertionIndex(activeItem);
        if (targetIndex >= 0) {
          const nextWidgets = moveMissionControlEntry(
            widgets,
            activeItem.i,
            targetIndex
          );
          const canonicalLayout = deriveOrderedMissionControlLayout(
