...
export function WorkbenchWidgetActions({
  actions,
}: {
  ...
  const run = async (action: WidgetHeaderAction): Promise<void> => {
    ...
    try {
      ...
      await showAppAlert({
        ...
        title: t("workbench.widget.actionFailed"),
      });
    } finally {
    ...
    <TooltipProvider delayDuration={250}>
      ...
      >
        <div
          ...
          data-slot="workbench-widget-action-measurement"
        >
          {actions.map((action) => {
            const Icon = action.icon;
            return (
        ...
        <div
          ...
          data-slot="workbench-widget-action-display"
          ...
          {overflow.length > 0 ? (
            <DropdownMenu onOpenChange={setMenuOpen} open={menuOpen}>
              <DropdownMenuTrigger asChild>
                <Button
                  aria-label={t("workbench.widget.menu")}
