...
import type {
  ...
  WorkbenchGridSize,
  WorkbenchWidgetCategory,
} from "@shared/contracts/workbench.ts";
import { HOST_DEFAULT_WIDGET_SIZE } from "@shared/contracts/workbench.ts";
...
export interface WorkbenchLibraryItem {
  addedCount: number;
  category: WorkbenchWidgetCategory | null;
  description?: string;
...
export type WorkbenchLibraryFilter = string;

export function buildWorkbenchLibraryItems(input: {
  ...
  const items: WorkbenchLibraryItem[] = [];
...
export function collectLibraryFilters(
...
): WorkbenchLibraryFilter[] {
  const categoryOrder: WorkbenchWidgetCategory[] = [
    "agent",
    "vcs",
    "system",
    "analytics",
  ...
  const filters: WorkbenchLibraryFilter[] = ["all"];
...
function matchesQuery(item: WorkbenchLibraryItem, query: string): boolean {
  if (!query) {
    return true;
  }
  const haystack = [
