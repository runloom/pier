...
import {
  ...
  filterLibraryItems,
  type WorkbenchLibraryFilter,
  type WorkbenchLibraryItem,
} from "./workbench-library.ts";
...
interface WorkbenchLibraryDialogProps {
  items: readonly WorkbenchLibraryItem[];
  onAdd: (widgetId: string) => void;
  onOpenChange: (open: boolean) => void;
  open: boolean;
}

function filterIcon(filter: WorkbenchLibraryFilter): LucideIcon {
  if (filter === "all") {
    return LayoutGrid;
  }
  if (filter.startsWith("cat:")) {
    const category = filter.slice("cat:".length) as WorkbenchWidgetCategory;
...
function filterLabel(
  filter: WorkbenchLibraryFilter,
  items: readonly WorkbenchLibraryItem[],
  t: (key: string) => string
): string {
  if (filter === "all") {
    return t("workbench.library.filterAll");
  }
  if (filter.startsWith("cat:")) {
    return t(`workbench.library.category.${filter.slice("cat:".length)}`);
  }
  ...
  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        ...
        data-testid={`workbench-library-filter-${filter}`}
        isActive={active}
        onClick={onSelect}
        type="button"
      >
...
function LibraryItemCard({
  ...
  return (
    <button
      ...
      data-testid={`workbench-widget-picker-item-${item.id}`}
        ...
        <div className="mt-auto flex flex-wrap items-center gap-1.5 pt-2">
          ...
          {item.addedCount > 0 ? (
            <Badge variant="outline">
              <Check />
              {item.multiInstance
                ? t("workbench.library.addedCount").replace(
                    "{{count}}",
                    String(item.addedCount)
                  )
                : t("workbench.library.added")}
            </Badge>
          ) : null}
        </div>
      </div>
 ...
 * （由调用方在 onAdd 中收口）。
 */
export function WorkbenchLibraryDialog({
  items,
  onAdd,
  onOpenChange,
  open,
}: WorkbenchLibraryDialogProps) {
  const t = useT();
  const searchRef = useRef<HTMLInputElement>(null);
  const [filter, setFilter] = useState<WorkbenchLibraryFilter>("all");
  ...
  return (
    <Dialog onOpenChange={onOpenChange} open={open}>
      ...
      >
        <DialogHeader>
          <DialogTitle>{t("workbench.library.title")}</DialogTitle>
          <DialogDescription>
            {t("workbench.library.description")}
          </DialogDescription>
        </DialogHeader>
        <SidebarProvider
        ...
        >
            ...
            <SidebarContent className="overflow-visible">
              ...
              {pluginFilters.length > 0 ? (
                <SidebarGroup className="p-0">
                  <SidebarGroupLabel>
                    {t("workbench.library.pluginGroup")}
                  </SidebarGroupLabel>
                  <SidebarGroupContent>
                    <SidebarMenu>
                      {pluginFilters.map((f) => (
          ...
          <main className="flex h-full min-h-0 flex-1 flex-col gap-3">
            <InputGroup className="shrink-0">
              ...
              <InputGroupInput
                data-testid="workbench-library-search"
              ...
              {visibleItems.length > 0 ? (
                ...
                <Empty className="h-full border-0 py-8">
                  <EmptyHeader>
                    ...
                    <EmptyTitle>{t("workbench.library.noResults")}</EmptyTitle>
                    <EmptyDescription>
                      {t("workbench.library.noResultsHint")}
                    </EmptyDescription>
                  </EmptyHeader>
                </Empty>
              )}
