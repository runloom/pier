...
export function resolveResponsiveGridCols(viewportWidth: number): number {
  ...
  return Math.max(
    ...
    Math.min(WORKBENCH_GRID_COLS, measured)
  );
}
 ...
 */
export function deriveOrderedWorkbenchLayout(
...
export function workbenchLayoutRows(layout: readonly LayoutItem[]): number {
  return layout.reduce((bottom, item) => Math.max(bottom, item.y + item.h), 0);
}

export function moveWorkbenchEntry(
  entries: readonly WorkbenchPanelWidgetEntry[],
  instanceId: string,
  targetIndex: number
...
export function resolveWorkbenchInsertionIndex(
  entries: readonly WorkbenchPanelWidgetEntry[],
  input: {
  ...
  for (let index = 0; index <= remaining.length; index++) {
    ...
    const layout = deriveOrderedWorkbenchLayout(candidateEntries, {
      cols: input.cols,
