...
export interface TerminalNativeLifecycleEffectArgs {
  ...
  monoFontFamilyRef: { current: string };
  panelId: string;
  retryNonce: number;
  ...
  translateRef: { current: TFunction };
}

export function runTerminalNativeLifecycleEffect({
  anchorRef,
  api,
  backgroundCreate,
  ...
  skipNativeCreate,
  translateRef,
}: TerminalNativeLifecycleEffectArgs): () => void {
