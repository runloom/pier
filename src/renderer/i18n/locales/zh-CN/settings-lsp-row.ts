...
export const settingsLspRow = {
  ...
  lspHostSectionDesc:
    "控制是否启动语言服务器。文件识别与高亮已内建。缺少的服务器见下方列表；打开对应文件时，编辑器状态栏也会提示安装命令。上方「编辑器语言功能」只决定「文件」是否使用这些服务。",
  lspEnabled: "运行语言服务器",
  ...
  lspToolsTitle: "本机语言服务器",
  lspToolsDesc:
    "补全、跳转和诊断使用你电脑上已有的语言服务器。没有的需要自行安装，Pier 不会代为下载。",
  lspToolsLoading: "正在检查语言服务器…",
  lspToolsEmpty: "无法检查语言服务器",
  lspToolsEmptyDesc: "请稍后重试，或重启 Pier 后再打开此页。",
  lspToolsNone: "没有可列出的语言服务器",
  lspToolsNoneDesc: "当前没有可检查的语言服务器。",
  lspToolsStatusBundled: "内置",
  lspToolsStatusAvailable: "已安装",
  lspToolsStatusMissing: "未安装",
  lspToolsInstallLabel: "安装",
