...
import { navigateFilesLspDefinition } from "./files-lsp-definition-navigate.ts";
import { parseFilesLspDefinitions } from "./files-lsp-definitions.ts";
import {
...
} from "./files-lsp-hover-data.ts";
import type { FilesLspHoverCardSession } from "./files-lsp-hover-preview.ts";
import type { FilesLspHoverRequests } from "./files-lsp-hover-requests.ts";
import type {
  FilesLspHoverInput,
  FilesLspPreparedDefinition,
} from "./files-lsp-hover-types.ts";
import { filesLspHoverModelHasContent } from "./files-lsp-hover-types.ts";

function clientHasCapability(client: object, name: string): boolean {
...
export async function clickDefinition(
  ...
  if (definitions.targets.length === 0) {
    mapping.destroy();
    host._mapping = null;
    // No toast for empty: matches VS Code (no-op). User already got mousedown feedback via selection cancel.
    return;
  }
  if (definitions.targets.length === 1 && definitions.targets[0]) {
    const target = prepareFilesLspDefinitions({
      targets: [definitions.targets[0]],
    })[0];
    if (!target) {
      mapping.destroy();
      return;
