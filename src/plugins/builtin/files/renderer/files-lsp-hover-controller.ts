...
} from "./files-lsp-definition-navigate.ts";
import {
  clickDefinition,
  requestTransient,
  runManual,
} from "./files-lsp-hover-controller-actions.ts";
import {
  clientHasCapability,
...
} from "./files-lsp-hover-controller-base.ts";
import {
...
import type { FilesLspPreparedDefinition } from "./files-lsp-hover-types.ts";
...
export class FilesLspHoverController extends FilesLspHoverControllerBase {
  ...
  async _requestTransient(
    ...
    return requestTransient(this, mode, candidate);
  }
  ...
  async _runManual(
    ...
    return runManual(this, plugin, candidate);
  }

  async _clickDefinition(candidate: FilesLspHoverCandidate): Promise<void> {
    return clickDefinition(this, candidate);
  }
  ...
  async _activateDefinition(target: FilesLspPreparedDefinition): Promise<void> {
