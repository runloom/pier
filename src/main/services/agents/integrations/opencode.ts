...
const OPENCODE_EMITTED_MAPPINGS = [
  ...
  { nativeEvent: "session.idle", pierEvent: "ActivityIdle" },
  ...
  { nativeEvent: "session.status=idle", pierEvent: "ActivityIdle" },
 ...
 * session.created→SessionStart, session.idle→ActivityIdle, session.error→error,
 ...
 * (idle)→ActivityIdle：已核对 1.18.29 的原生 SessionStatus。
 * idle 只更新当前忙闲，不生成回合结果或完成通知；普通 Stop 仍是 advisory。
...
export function buildOpencodePluginSource(
  ...
  // 同源提供方的安装版本必须单独核对，不能因复用生成器就自动升权。
  const idleEvent = pluginId === AGENT_ID ? "ActivityIdle" : "Stop";
    ...
    event: ({ event }) => {
      ...
      if (event.type === "session.created") {
        ...
        if (child && (state === "busy" || state === "retry"))
          ...
          pierEmit("${idleEvent}", event.type + "=idle", event, { nativeState: state });
        ...
        else pierEmit("${idleEvent}", event.type, event);
