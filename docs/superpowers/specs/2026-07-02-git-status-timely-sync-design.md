...
- **节奏**：默认开启；间隔 5 分钟；仅应用窗口聚焦时执行；重新聚焦且距上次成功 fetch
  超过间隔时立即补一次——该补跑还受 60s 地板保护（距上次尝试 ≥60s 才放行），避免
  聚焦/失焦抖动触发重试风暴；命中鉴权冷却期间（见下）不受聚焦补跑豁免。
- **护栏**：
  - 禁止一切凭据交互：`GIT_TERMINAL_PROMPT=0`（HTTP）+ `GIT_SSH_COMMAND="ssh -oBatchMode=yes"`
    （SSH，passphrase/known_hosts 询问直接失败而不是挂起）；
  - 单次超时 30s，超时杀进程；
  - 每仓库串行（上一次未结束不叠加）；
  - 连续失败指数退避（5min → 10min → 20min，上限 40min）；真实鉴权错误（凭据类
    stderr 判别，裸 "permission denied" 不算）触发 60 分钟冷却而非永久停用，
    冷却到期自动恢复尝试，仅 console 记日志。
