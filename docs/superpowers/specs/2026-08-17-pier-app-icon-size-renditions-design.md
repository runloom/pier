...
继续使用项目锁定的 electron-builder 官方图标工具分别生成 F 与 I 的完整临时 ICNS，再由项目脚本按 ICNS 条目类型合成最终文件：

- I Micro：`icp4`、`icp5`、`icp6`、`ic07`、`ic11`、`ic12`。
- F 标准稿：`ic08`、`ic09`、`ic10`、`ic13`、`ic14`。

对应关系覆盖 16、32、64、128、256、512、1024px 及 Retina 条目。合成脚本必须校验：

- ICNS 文件头与总长度一致。
- 目标条目类型齐全且不重复。
- 每个条目包含有效 PNG 数据。
- PNG 像素尺寸与 ICNS 条目语义一致。

不使用当前系统的 `iconutil` 重新封装，因为该主机版本无法稳定重编译已验证的 iconset；官方工具仍负责 SVG 栅格化和每档缩放，项目脚本只做确定性的 ICNS 条目选择与封装。
...
4. 解析最终 ICNS，验证条目类型、PNG 尺寸和来源分配。
5. 检查 16、32、64、128、256、512、1024px 渲染：小尺寸来自 I，大尺寸来自 F。
6. 对照已确认的 I Dock 合成图，检查终端与港湾间距、港湾底厚、光斑清理和整体占比。
7. 检查 Windows ICO 与 Linux PNG 保持透明背景且无裁切。
8. 执行项目格式与静态检查。
9. 由独立子智能体复核视觉、资源映射和生成脚本；若发现差异，修复后重新生成并复核，直到无关键、重要或次要问题。
