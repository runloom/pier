...
| 界面映射 | ❌ 禁止 | 不投像素、不复刻 dockview、不做「缩小的桌面」。移动端按自己的信息架构重排（总览列表 → 详情）。**会话视图 = 终端投影，不做聊天对话框**——Pier 是终端工作台，宿主不提供 transcript/会话消息 API（AGENTS.md 明确），聊天形态在架构上没有数据来源；业界同样以终端视图为默认（Orca 移动端默认终端、Chat UI 仅可选；Moshi 默认终端，已提供基于主机会话记录的实验性 Chat View（2026-09-05 核对）；这不改变 Pier 当前没有对应聊天接口的边界；herdr-relay 终端+快捷面板） |
...
- **二维码只出现在桌面。** 无相机时走「粘贴配对内容」文字级退路（不做相册读图），不把「填写主机地址」画进主路径。
