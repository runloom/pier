...
工作台采用始终自动生效的响应式有序网格：删除后立即压实，添加追加到末尾，拖拽只改变阅读顺序，调整尺寸只改变用户的 `w/h` 偏好。Dockview 宽度变化自动换行，不写持久化参数，也不产生普通横向滚动。
...
这些复杂度没有为本地开发工作台带来相称价值。工作台首先是信息仪表盘，稳定阅读顺序、自动适配窗口和低操作成本更重要。
...
```text
Dockview params
  → salvageWorkbenchPanelParams（v3 原顺序 / 旧版 y-x 转序）
  → canonicalizeWorkbenchPanelParams（去重、尺寸约束）
  → useWorkbenchPanelState（唯一语义状态）
  → 容器宽度换算 2..12 列
  → deriveOrderedWorkbenchLayout（临时 x/y）
  → RGL 渲染
...
- 策略：`workbench-ordered-layout.ts`
- 状态：`workbench-panel-state-normalization.ts`、`use-workbench-panel-state.ts`
- UI：`workbench-panel.tsx`、`workbench-context-menu.ts`、`workbench-settings-dialog.tsx`
...
| 需求 | 代码证据 | 测试证据 |
| --- | --- | --- |
| 删除自动压实 | 数组删除 + 统一派生布局 | `use-workbench-panel-state.test.tsx`、`workbench-ordered-layout.test.ts` |
...
| 旧数据安全迁移 | `salvageWorkbenchPanelParams` | 共享契约和状态单元测试 |
| 设置使用弹窗 | `WorkbenchSettingsDialog` | 物料库组件测试 |
