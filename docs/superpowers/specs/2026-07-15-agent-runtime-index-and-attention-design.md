...
2026-09-07 后续修订：[工作台骨架：工作树 tile 与主 / 子窗口](./2026-09-07-workbench-worktree-tiles-design.md) 确认主窗口带常驻侧栏（L0 索引），本文 §1.5「Agents 侧栏不适用 Pier」与 §6.1「无常驻侧栏」边界据此作废；Index facade / `focus` / 双通道全部保留，`focus` 目标改为「工作树 → tile → 视图」（该文 §9.4 / §13）。标题栏短列表的 `limit: 8` 截断取消，Quick Pick 保留为子窗口与侧栏折叠态的同源入口。
