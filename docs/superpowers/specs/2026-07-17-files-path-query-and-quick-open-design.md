...
- 保留侧栏搜索条与 `pier.files.treeSearch`  
- **删除/收缩** `files-tree-search-loader` 的「为搜索灌整树」职责  
- 有 query 时：  
  - 树本体可保持当前展开态，或切换为「路径结果列表」层（推荐：**独立结果列表**，避免 Pierre hide-non-matches 依赖全量 path set）  
  - 结果来自同一 path query  
  - 选择结果：打开 + reveal 祖先链  
- loading 文案保留「正在搜索…」，但 **完成前也可渐进 batch 更新**；完成前不得声称「无结果」除非已 `done` 且 items 空  
- 匹配计数 = 当前累计/最终 top-K 数量（需在 UI 标明截断，如 `200+` 当 `truncated`）  

### 6.3 与 Pierre 内置搜索关系

- 不再依赖 Pierre `setSearch` 对 listed paths 的 `includes` 作为主搜索引擎  
- 若保留 Pierre 高亮，仅用于已 reveal 子树的可选装饰；主结果源是 FileQuery  
...
- quick open：输入、loading、选择打开  
- 树搜索结果层：渐进 batch、完成空态、截断提示  
...
| P2 | 树搜索改接 path query；移除/收缩整树 search loader | 搜 `theme.ts` 稳定出 `code-mirror-editor-theme.ts`；store 节点不因搜索暴涨 |
...
| 树 UI 从 filter-tree 改为结果列表 | 保留打开/reveal 旧语义；组件测试锁交互 |
...
3. `Cmd+P` 与树搜索对同一 query 的 top 结果集合与排序一致（允许 UI 展示密度不同）。  
4. 快速连打字无旧结果闪回。  
