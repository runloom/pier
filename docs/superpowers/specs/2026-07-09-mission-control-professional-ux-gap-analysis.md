...
- `2026-07-05-dashboard-kit-and-codex-accounts-design.md`（大盘 / 指挥中心地基）
- `2026-07-06-dashboard-responsive-layout-design.md`（固定格宽 + 窄容器派生）
- `docs/superpowers/plans/2026-07-09-mission-control-grid-interaction.md`（默认尊重持久化布局；整理仅显式触发）

## 1. 对标边界

### 1.1 定位

Pier 指挥中心是本地 AI 工作台里的「活动与资源总览」，不是 BI 报表画布。对标的是
...
P0 做完后指挥中心应不再被误判为「功能藏在右键里的 demo」。
...
- 实例模型 v2（`id` / `widgetId` / `params`）、`salvageMissionControlPanelParams`
...
| 区域 | 路径 |
| --- | --- |
| 面板壳 / 网格 / 右键 | `src/renderer/panel-kits/mission-control/mission-control-panel.tsx` |
| 卡片 chrome | `src/renderer/panel-kits/mission-control/mission-control-widget-card.tsx` |
| 添加入口 | `src/renderer/panel-kits/mission-control/mission-control-add-card.tsx` |
| 状态写回 | `src/renderer/panel-kits/mission-control/use-mission-control-panel-state.ts` |
| 物料库 | `src/renderer/panel-kits/mission-control/mission-control-library-dialog.tsx` |
| 系统资源 | `src/renderer/panel-kits/mission-control/core-widgets/system-resources-widget.tsx` |
| 活动总览 | `src/renderer/panel-kits/mission-control/core-widgets/activity-widget.tsx` |
| 错误边界 | `src/renderer/panel-kits/mission-control/mission-control-widget-error-boundary.tsx` |
| 契约 | `src/shared/contracts/mission-control.ts` |
| 组件测试 | `tests/component/mission-control-panel.test.tsx` |
