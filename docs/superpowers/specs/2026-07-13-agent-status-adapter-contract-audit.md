...
> 现行状态规则以 [2026-09-06 状态证据与回合归属](2026-09-06-agent-status-evidence-gold-standard.md) 为准：候选不清空状态或工作集，旧回合不能结算新回合，无 PromptSubmit 不能证明父子关系。本文保留原审计与历史决策。
...
| claude | Claude 式嵌套 JSON hook | `full` | `SessionStart→SessionStart`；`UserPromptSubmit→PromptSubmit`；`PreToolUse→ToolStart/InteractionRequested`；`PostToolUse→ToolComplete/InteractionResolved`；`PostToolUseFailure→ToolComplete/InteractionResolved`；`PreCompact→MaintenanceStarted`；`PostCompact→MaintenanceCompleted`；`Stop→Stop`；`StopFailure→error`；`Notification→TurnCompleted`；`SubagentStart/SubagentStop/SessionEnd→同名` | `advisory` | 无 |
...
| kimi | TOML hook | `full` | `SessionStart/SessionEnd→同名`；`UserPromptSubmit→PromptSubmit`；`PreToolUse→ToolStart/InteractionRequested`；`PostToolUse→ToolComplete/InteractionResolved`；`PostToolUseFailure→ToolComplete/InteractionResolved`；`PermissionRequest→InteractionRequested`；`PermissionResult→InteractionResolved`；`PreCompact/PostCompact→processing`；`Stop→Stop`；`StopFailure→Stop`；`Interrupt→Stop`；`SubagentStart/SubagentStop→同名` | `none` | 无 |
