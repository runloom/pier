...
抄错族就会出现：悬停藏分割线（Chrome 靠凸起形状切开，Pier 没有这层形状）、把短线画在内容 padding 里（两边空隙不对称）。
...
| 贴悬停 | 藏（悬停也有凸起形状） | 藏 | **不藏**（悬停只是底色微亮） | **不藏** |
...
Chrome / loomdesk 窗口条在悬停时藏线，是因为悬停 tab **已经变成一块独立形状**。Pier 悬停只有一层洗底，藏线会让邻居重新糊成一块——这是上一轮错误的局部抄袭。
...
| K4 | 分割线 **只**在两颗 inactive 之间显示。隐藏：条内第一颗；选中 tab 自己的线；选中右侧邻居的线 | 选中已有内容底，线是多余双边缘 |
| K5 | **悬停不藏线** | 悬停没有独立形状；线仍承担邻居切开 |
...
- `tests/unit/renderer/styles/dockview-tab-focus-css.test.ts`：锁 K1–K8 选择器 / padding / `--list-hover-bg` / 禁止 hover 藏线
- `tests/component/app/panel-tab-header.test.tsx`：锁真实节点存在

实现禁令：禁止再把 Chrome 窗口条的「悬停藏线」搬到面板 tab；禁止把短线 `left` 写进内容 padding 里用负值猜接缝。
