...
| 1 文档读写安全边界 | 安全写入主链已落地 | `file-document-codec.ts`、`file-document-reader.ts`、`file-safe-writer.ts`、`file-path-transaction-lock.ts` 及对应测试；`list` 按 symlink 目标归入 directory/file | 路径操作需补规范身份锁与最终符号链接语义；macOS ACL、扩展属性和所有者策略需发布验证 |
| 2 renderer 文档模型 | 进行中 | 编码、换行、修订、只读、耐久未知和磁盘删除状态已进入统一文档模型；外部删除后 Save 以 `expected: absent` 重建（单测） | 外部删除重建的端到端与“目标已被他人新建”产品文案仍可加强 |
...
| 0B、6–10 发现与规模 | 部分落地 | 主进程 path query、`Cmd+P` 快速打开、树搜索物化进既有树 UI（见 2026-07-17 / 07-18 specs） | 项目内容搜索、大目录分页、搜索运行时打包一致性与双架构安装包验证 |
| 11、14 查看与比较 | 部分落地 | PNG/JPEG/GIF/WebP 预览协议 + 二进制 Empty；保存冲突行级 Compare | 通用任意两文件差异视图；更完整的二进制摘要 |
| 12、13 编辑与树操作补全 | 待实施 | 现有 CodeMirror 编辑和基础树操作继续保留 | 文件级设置、剪切粘贴、排序、全部折叠和批量结果治理 |

当前结论：P0 数据安全主链可用，但仍不能宣称发布级完成（强杀/双窗口/安装包恢复证据不足）。发现轮的 **路径查询与快速打开已落地**；**项目内容搜索与目录分页仍未做**。图片预览已落地；通用 Diff 与树剪切粘贴等仍属后续。P1/P2 不阻塞先完成 P0 剩余证据，但不得混入 P0 的数据安全承诺。
