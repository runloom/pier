...
- 普通 diff：树点击只回 hunk。折叠的未修改行是可聚焦按钮；激活后按**同一**已绘制 `indexRevision` 拉两侧全文再展开。修订不在快照里时结果是 `indexMoved`，加载器保持等待，不重试、不改读实时索引。
- 冲突：一文一屏。标记文本只挂 `UnresolvedFile`。工作区还有可读文本时，动作卡旁边是工作区 `File`。工作区文件不存在且两侧 stage 可读时，两侧 diff 是唯一正文。过大、编码不对、读失败且文件还在时，「打开文件」是主按钮。
