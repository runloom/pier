...
- **xAI**：Grok 4、Grok 4.x fast、Grok Code。
...
**每周一 UTC 03:00**，`.github/workflows/update-model-pricing.yml` cron 触发：
...
2. 拉 [OpenRouter `/v1/models`](https://openrouter.ai/api/v1/models)（用于补 alias）
...
7. 有变化就自动开 PR，标 `chore` + `pricing` label，reviewer 检查后合并
8. PR 里的 `pricing-diff.md` 单独列出**可疑变化**（>= 5x 涨或 <= 20% 跌），提醒 reviewer 二次核对
...
| 源 | 用途 | 更新频率 | 覆盖 |
|---|---|---|---|
| [LiteLLM canonical JSON](https://raw.githubusercontent.com/BerriAI/litellm/main/model_prices_and_context_window.json) | 主要单价来源 | 社区共建，日多次 | 3000+ entries |
| [OpenRouter `/v1/models`](https://openrouter.ai/api/v1/models) | 补 alias（`provider/canonical` 形式） | 实时 | 200+ entries |
| 厂商官方定价页 | 冲突仲裁 fallback | 变化时手工 | 全部 |

冲突仲裁：LiteLLM 与 OpenRouter 差异 → 以厂商官方为准，脚本记录到 diff 报告的 "Suspicious deltas" 让 reviewer 人工判定。
