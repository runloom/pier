...
**Architecture:** git-watch-service 从 3 签名扩为 4 签名（新增 refsSig、增强 worktreeSig）；新建 git-autofetch-service 只执行 `git fetch --prune` 不产出状态（变化经既有签名→广播管道流向 renderer）；getStatus 增加 `mergedIntoDefault` 检测。Spec：`docs/archive/superpowers/specs/2026-07-02-git-status-timely-sync-design.md`。
