...
- 官方受管理外部插件：位于 `packages/plugin-*`，通过签名官方索引、包校验、不可变版本目录和启动时运行态快照加载，例如 `pier.claude`、`pier.codex`、`pier.grok`、`pier.ssh`，以及可选语言包 `pier.lsp-zig` / `pier.lsp-java` / `pier.lsp-cpp` / `pier.lsp-csharp`（安装 / 卸载，不随 builtin 常驻）。
