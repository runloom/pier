...
文档索引：[`README.md`](./README.md)。发布通道总览与流程图：[`release.md`](./release.md)。本文侧重范围、开发、打包与运行时校验。
...
- 官方受管理外部插件：位于 `packages/plugin-*`，通过签名官方索引、包校验、不可变版本目录和启动时运行态快照加载，例如 `pier.claude`、`pier.codex`、`pier.grok`、`pier.ssh`、`pier.agent-splits`。语言高亮与 PATH 语言服务在 Files / 宿主 L0 内建，不走独立语言包插件。退役 id（例如已更名为 `pier.agent-splits` 的 `pier.tmux`）不得出现在签名索引或设置插件列表，见 [`superpowers/specs/2026-09-04-retired-managed-plugin-invisibility-gold-standard.md`](./superpowers/specs/2026-09-04-retired-managed-plugin-invisibility-gold-standard.md)。
...
pnpm plugin:agent-splits:build    # 构建智能体分屏官方插件
pnpm plugin:agent-splits:pack     # 构建并生成 tgz 与 sha256
...
- 额外的同目录 `dist/*.js`（例如 `pier.agent-splits` 的 `dist/tmux.js`）
