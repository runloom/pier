...
test.describe("Terminal task status e2e", () => {
  test("keeps runtime control after a failed background run for intentional re-run/close", async () => {
    ...
    try {
      ...
      await expect
        ...
        .toBe(true);
      // Product keeps the bar after finish (no finish toast); user re-runs or closes.
      await expect(control).toHaveAttribute("data-run-status", "failed", {
        timeout: 10_000,
      });
      await expect(
        win.getByTestId("terminal-runtime-control-restart")
      ).toBeVisible();
      await expect(
        win.getByTestId("terminal-runtime-control-close")
      ).toBeVisible();
    } finally {
      await app.close();
  ...
  test("keeps runtime control after a successful background run for intentional re-run/close", async () => {
    test.setTimeout(120_000);
  ...
  test("renderer rerun and stop actions keep the task tab and runtime control on one run", async () => {
    ...
    try {
      ...
      await expect
        ...
        .toBe("cancelled");
      // 优雅停止会 dismiss 控制条；tab 仍记取消态，快捷键可再跑。
