...
test.describe("Startup stability e2e", () => {
  ...
  test("vetoes a real close once when durable draft protection fails, then retries", async () => {
    ...
    // Monorepo root tree is virtualized; package.json is off-screen among many
    // dirs. A tiny workspace keeps the draft file in the initial viewport.
    const workspaceDir = mkdtempSync(join(tmpdir(), "pier-close-draft-ws-"));
    writeFileSync(join(workspaceDir, "draft.txt"), "draft base\n");
    let application: ElectronApplication | null = null;
    ...
    try {
      ...
      await page
        ...
        .click();
      await page.getByText("draft.txt", { exact: true }).first().click();
