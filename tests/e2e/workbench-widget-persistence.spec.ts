 ...
 * 尚未注册，CDP 按键会被丢掉）。
 *
 * 仅适用于"终端是活跃面板"的首启默认布局；恢复阶段活跃 tab 是工作台时，
...
/** 经命令面板执行"新建工作台"（renderer action 无 preload 直调通道，命令面板是标准入口）。 */
async function openWorkbenchViaPalette(win: Page): Promise<void> {
  await win.keyboard.press("Meta+Shift+KeyP");
  await expect(win.locator("[cmdk-input]")).toBeVisible({ timeout: 10_000 });
  const item = win.locator("[cmdk-item]").filter({ hasText: "新建工作台" });
  await expect(item).toBeVisible({ timeout: 10_000 });
  await item.click();
}

async function addWidget(win: Page, widgetId: string): Promise<Locator> {
  await win.locator('[data-testid="workbench-add-widget"]').click();
  await win
...
test.describe("Workbench widget persistence e2e", () => {
  test("新建工作台 → 添加 core activity widget → 重启 → 组装恢复", async () => {
    const userDataDir = mkdtempSync(join(tmpdir(), "pier-workbench-e2e-"));
    try {
      // 第一次启动：新建工作台 + 添加 widget
      const firstApp = await launchPierApp(userDataDir);
      const firstWindow = await firstApp.firstWindow();
      await waitForAppShellReady(firstWindow);

      await openWorkbenchViaPalette(firstWindow);

      // 空态工作台出现（Phase 1 workbench-panel.tsx 的空态 testid）
      await firstWindow.waitForSelector('[data-testid="workbench-empty"]', {
        timeout: 5000,
      });

      // 点击"添加组件"，选择 core.activity-overview
      await firstWindow.locator('[data-testid="workbench-add-widget"]').click();
      await firstWindow
        .locator(
          '[data-testid="workbench-widget-picker-item-core.activity-overview"]'
        )
        .click();

      // 验证 widget 卡片出现
      await firstWindow.waitForSelector(
        '[data-testid="workbench-widget-core.activity-overview"]',
        { timeout: 5000 }
      );
      ...
      try {
        ...
        // 恢复的布局里工作台是活跃 tab，终端在后台 tab（dockview 不挂后台
  ...
  test("排序和尺寸偏好在重启后恢复，响应式坐标不持久化", async () => {
    ...
    try {
      ...
      const handle = activity.locator(".workbench-widget-drag-handle");
      ...
      try {
        ...
        const order = await grid.evaluate((element) =>
          [...element.querySelectorAll("[data-workbench-instance-id]")].map(
            (item) => item.getAttribute("data-workbench-instance-id")
          )
        );
