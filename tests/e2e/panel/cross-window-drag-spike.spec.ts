...
async function elementLocalPoint(
  ...
  if (page.isClosed()) {
    throw new Error(`Page closed before locating ${selector}`);
  }
...
test.describe("Panel cross-window drag spike", () => {
  test("gate: real MIME proof or exclusive native-monitor path", async () => {
    ...
    try {
      ...
      // Best-effort only: after multi-window continuous drag, Chromium may drop
      // the source document briefly (no .dv-tab). Gate decision below does not
      // depend on this probe succeeding.
      try {
        if (!source.isClosed()) {
          await focusHostWindow(app, hosts.source.id);
          await source.bringToFront();
          await expect(source.locator(".dv-tab").first()).toBeVisible({
            timeout: 10_000,
          });
          const tabPoint = await elementLocalPoint(source, ".dv-tab", "center");
          await source.mouse.move(tabPoint.x, tabPoint.y);
          await source.mouse.down();
          await source.mouse.move(tabPoint.x + 30, tabPoint.y + 4, {
            steps: 6,
          });
        ...
        console.log(
          "[panel-dnd-spike] escape mid-drag probe skipped:",
      ...
      // Under full-suite stress, macOS may stack or overlap windows so the
      // geometric midpoint still hits a frame — log and continue; farOutside
      // already proves the outside branch.
      if (classification.between.kind !== "outside") {
        console.log(
          "[panel-dnd-spike] between-windows point still hit a frame (layout stress); farOutside covers outside rule",
          JSON.stringify(classification.between)
        );
      }
      ...
      // Escape mid-drag is best-effort (multi-window stress may drop the
      // source tab surface). Gate path decision does not require it.
      if (sourceLog.keydownEscape === 0) {
        console.log(
          "[panel-dnd-spike] keydownEscape not observed after mid-drag probe; continuing with path gate"
        );
      }
