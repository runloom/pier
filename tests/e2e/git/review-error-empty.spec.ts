...
test("index load failure renders an Empty error state with retry", async () => {
  ...
  try {
    ...
    // Warm open so status slots and panel kit are ready.
    await openChangesPanel();
    await expect(page.getByTestId("pierre-diff-root")).toBeAttached({
      timeout: 30_000,
    });
    ...
    // Dirty working tree still needs the modified file after index rebuild.
    writeFileSync(join(sourceDirectory, "app.ts"), "export const value = 1;\n");
    // Toast may also expose 重试; drive the Empty-state Retry only.
    await page
      .locator('[data-slot="error-empty"]')
      .getByRole("button", { name: /Retry|重试/u })
      .click();
    await expect(page.getByTestId("pierre-diff-root")).toBeAttached({
      timeout: 30_000,
    });
