...
export async function openMissionControl(win: Page): Promise<void> {
  await openPaletteAction(win, /新建指挥中心|New Mission Control/);
  await expect(
    win.locator('[data-testid="mission-control-grid-wrapper"]')
...
export async function addWidget(win: Page, widgetId: string): Promise<Locator> {
  ...
  const add = win.locator('[data-testid="mission-control-add-widget"]');
  await add.scrollIntoViewIfNeeded();
  await add.click();
  await expect(
    win.locator('[data-testid="mission-control-library"]')
  ).toBeVisible();
  await win
...
export async function resizeToBoundary(
  ...
  const wrapper = win.locator('[data-testid="mission-control-grid-wrapper"]');
  ...
  if (!(gridBox && Number.isInteger(cols) && cols > 0)) {
    throw new Error("Mission Control live grid geometry is unavailable");
  }
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (!current) {
      throw new Error("Mission Control grid item has no bounding box");
    }
    ...
    if (!box) {
      throw new Error("Mission Control resize handle has no bounding box");
    }
