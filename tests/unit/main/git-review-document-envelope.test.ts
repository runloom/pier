...
describe("Git Review patch envelope", () => {
  it("接受 typechange 作为 modified，并按 hunk 计算统计", () => {
    const material = materialFromGitReviewPatchEnvelope(
  ...
  it("拒绝多文件 raw envelope 与错误路径，不能把 pathspec 回归带入 UI", () => {
    ...
    expect(() =>
      materialFromGitReviewPatchEnvelope(multiple, options())
    ).toThrow(GitReviewDocumentProtocolError);
    expect(() =>
      materialFromGitReviewPatchEnvelope(wrongPath, options())
  ...
  it("只把 Git 协议完整行识别为 binary，不误判源码中的标记字样", () => {
    const text = materialFromGitReviewPatchEnvelope(
      envelope(
    ...
    const binary = materialFromGitReviewPatchEnvelope(
      envelope(
        [`:100644 100644 ${oid} ${oid} M`, "file.ts"],
        "diff --git a/file.ts b/file.ts\nGIT binary patch\nliteral 0\nHcmV?d00001\n"
      ),
