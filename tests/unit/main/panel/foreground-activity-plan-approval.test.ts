...
describe("阻塞问卷/审批的 ToolStart 不得谎报 tool", () => {
  it.each([
    "ask",
    "ask_user",
    "ask_user_question",
    ...
    "exit_plan_mode",
    "ExitPlanMode",
  ])("ToolStart(%s) 进入 waiting，而不是 tool", (toolName) => {
    const aggregator = createRawForegroundActivityAggregator();
  ...
  it("具名 ask ToolComplete 结束问卷 waiting", () => {
    const aggregator = createRawForegroundActivityAggregator();
    ingest(aggregator, toolStart("ask-1", "ask"));
    expect(statusOf(aggregator)).toBe("waiting");
    ingest(aggregator, toolComplete("ask-1", "ask"));
    expect(statusOf(aggregator)).toBe("processing");
    aggregator.dispose();
  });

  it("无名 ask_user ToolStart / ToolComplete 也能闭环，不卡在 waiting", () => {
    const aggregator = createRawForegroundActivityAggregator();
    ingest(aggregator, {
      agent: "gemini",
      event: "ToolStart",
  ...
  it("仅 ToolStart 出现的问卷可被后续普通工具顶替", () => {
    const aggregator = createRawForegroundActivityAggregator();
    ingest(aggregator, toolStart("ask-1", "ask"));
    expect(statusOf(aggregator)).toBe("waiting");
  ...
  it("ToolStart 问卷之后的具名 InteractionRequested 不再被普通工具顶替", () => {
    const aggregator = createRawForegroundActivityAggregator();
  ...
  it("新回合 PromptSubmit 清掉没有结果事件的 ask_user", () => {
    const aggregator = createRawForegroundActivityAggregator();
    ingest(aggregator, toolStart("ask-1", "ask_user"));
    expect(statusOf(aggregator)).toBe("waiting");
    ingest(aggregator, {
      agent: "grok",
      event: "PromptSubmit",
