...
import { materialFromGitReviewPatchEnvelope } from "@main/services/git-review/document/envelope.ts";
import {
  GitReviewPatchEnvelopeSelector,
  selectGitReviewPatchEnvelope,
} from "@main/services/git-review/document/envelope-selector.ts";
import {
  GitReviewDocumentProtocolError,
  GitReviewDocumentStaleError,
  readGitReviewPatch,
} from "@main/services/git-review/document/patch.ts";
import {
...
describe("Git Review patch envelope", () => {
  ...
  it("git 输出为空（文件在扫描后被还原/改名）按陈旧事实处理", () => {
    // 索引扫描后文件被还原/改名/取消暂存：git 对 pathspec 输出 0 字节，
    // 没有 raw 段也没有分隔符。这是竞态陈旧事实，必须走 stale 重试，
    // 不能当作不可恢复的协议错误打断用户。
    const selector = new GitReviewPatchEnvelopeSelector(options().fact);

    expect(() => selector.finish()).toThrow(GitReviewDocumentStaleError);

    const patchOnly = new GitReviewPatchEnvelopeSelector(options().fact);
    patchOnly.push(
