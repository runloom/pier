...
} from "@main/services/git-review/path/guard.ts";
import { raceGitReviewPathOperation } from "@main/services/git-review/path/operation.ts";
import { afterEach, describe, expect, it, vi } from "vitest";

const { mockPathOpen } = vi.hoisted(() => ({
  mockPathOpen:
    vi.fn<
      typeof import("@main/services/git-review/path/open.ts").openGitReviewFileNoSymlinks
    >(),
}));

vi.mock("@main/services/git-review/path/open.ts", async (importOriginal) => {
  const actual =
    await importOriginal<
      typeof import("@main/services/git-review/path/open.ts")
    >();
