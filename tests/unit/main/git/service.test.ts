...
describe("createGitService", () => {
  ...
  it("stage 跳过被 ignore 的未跟踪路径且不抛错", async () => {
    ...
    const service = createGitService({
      execGit: (args) => {
        ...
        if (args[0] === "ls-files" && args.includes("--others")) {
          return Promise.resolve(".superpowers/secret.txt\0ok.ts\0");
        }
        if (args[0] === "check-ignore") {
  ...
  it("stage 跳过 worktree 中不存在的未跟踪 pathspec（rename 旧路径）", async () => {
    const calls: Array<readonly string[]> = [];
    const service = createGitService({
      execGit: (args) => {
        calls.push(args);
        if (args[0] === "ls-files" && args.includes("--cached")) {
          // 新路径已在 index（rename 已部分 staged），旧路径不在
          return Promise.resolve("packages/ui/src/diff-view/input-store.ts\0");
        }
        if (args[0] === "ls-files" && args.includes("--others")) {
          // 旧路径不在 worktree，others 不列出
          return Promise.resolve("");
        }
        return Promise.resolve("");
      },
    ...
    await service.stage("/repo", {
      paths: [
        "packages/ui/src/diff-view/input-store.ts",
        "packages/ui/src/diff-view-input-store.ts",
      ],
    });

    const plainAdd = calls.filter(
      (args) => args[0] === "add" && args[1] === "--"
    );
    expect(plainAdd).toEqual([]);
    expect(calls.some((args) => args[0] === "add" && args.includes("-u"))).toBe(
      true
    );
  });
  ...
  it("stage 对 pathspec 未命中的已跟踪路径不抛错", async () => {
    const calls: Array<readonly string[]> = [];
    const service = createGitService({
      execGit: (args) => {
        calls.push(args);
        if (args[0] === "ls-files" && args.includes("--cached")) {
          return Promise.resolve("gone.ts\0ok.ts\0");
        }
        if (
          args[0] === "add" &&
          args.includes("-u") &&
          args.includes("gone.ts")
        ) {
          return Promise.reject(
            new Error(
              "git 退出码 128: fatal: pathspec 'gone.ts' did not match any files"
            )
          );
        }
        return Promise.resolve("");
  ...
  it("unstage 用 git restore --staged -- <paths>", async () => {
    ...
    const service = createGitService({
      execGit: (args) => {
        calls.push(args);
        if (args[0] === "ls-files" && args.includes("--with-tree=HEAD")) {
    ...
    expect(calls.at(-1)).toEqual(["restore", "--staged", "--", "src/a.ts"]);
  });

  it("unstage 恢复 index 与 HEAD 中存在的路径（删除/rename 旧路径），只跳过两边都不存在的 pathspec", async () => {
    const calls: Array<readonly string[]> = [];
    const service = createGitService({
      execGit: (args) => {
        calls.push(args);
        if (args[0] === "ls-files" && args.includes("--with-tree=HEAD")) {
          // index 含新路径，HEAD 含 rename 旧路径；ghost.ts 两边都不存在
          return Promise.resolve(
            "packages/ui/src/diff-view/input-store.ts\0packages/ui/src/diff-view-input-store.ts\0"
          );
        }
        return Promise.resolve("");
      },
  ...
  it("unstage 的 --with-tree=HEAD 无法解析时回退到 --cached 过滤", async () => {
    const calls: Array<readonly string[]> = [];
    const service = createGitService({
      execGit: (args) => {
        calls.push(args);
        if (args[0] === "ls-files") {
          if (args.includes("--with-tree=HEAD")) {
            return Promise.reject(new Error("fatal: tree-ish HEAD not found."));
          }
    ...
    await service.unstage("/repo", { paths: ["src/a.ts", "src/ghost.ts"] });
