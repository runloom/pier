...
describe("tmux cli guards", () => {
  it("prints version locally without hitting the control socket", async () => {
    ...
    for (const argv of [["-V"], ["-version"], ["version"]]) {
      const result = await runTmux(argv, { cwd: "/tmp", env: {}, invoke });
  ...
  it("completes wait-for locally and maps layouts without moving panes", async () => {
    ...
    // 确定性均分：每个非 leader 面板显式 1/N 高度（不再用 equalize）。
    expect(findCommand(layout.commands, "panel.equalize")).toBeUndefined();
    const heightRatios = layout.commands
      .filter((command) => command.type === "panel.setSize")
      .map((command) => command.heightRatio)
      .filter((ratio) => ratio !== undefined);
    expect(heightRatios.length).toBeGreaterThan(0);
    for (const ratio of heightRatios) {
