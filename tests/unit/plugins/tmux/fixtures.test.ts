...
import { loadSessionMap } from "../../../../packages/plugin-tmux/src/main/session-map.ts";
import type {
  ControlResult,
  JsonCommand,
} from "../../../../packages/plugin-tmux/src/tmux/types.ts";
import { isRecord } from "../../../../packages/plugin-tmux/src/tmux/types.ts";
import { shellInvokedCommand } from "../../../../packages/plugin-tmux/src/tmux/verb-context.ts";
import {
  runTmux,
  TMUX_VERSION_LINE,
...
describe("gold-path tmux fixtures", () => {
  it("Claude: split → send-keys → capture-pane → kill-pane", async () => {
    ...
    const killed = await runTmux(["kill-pane", "-t", "%1"], { env, invoke });
  ...
  it("omo: -V → split -P -F → split new pane -v → resize 30% → kill + equalize", async () => {
    ...
    const version = await runTmux(["-V"], { env, invoke });
    ...
    const killed = await runTmux(["kill-pane", "-t", "%2"], { env, invoke });
    expect(killed.exitCode).toBe(0);
    expect(findCommand(killed.commands, "terminal.close")).toMatchObject({
      panelId: "panel-new-2",
    });
