import { materializePathQueryHits } from "@plugins/builtin/files/renderer/search/path-query-materialize.ts";
import {
  clearFilesTreeStore,
...
describe("materializePathQueryHits", () => {
  it("loads only ancestor directories for hits, never whole-tree BFS", async () => {
    ...
    const list = vi.fn(async (_root: string, options?: { path?: string }) => {
      ...
      if (path === "src/plugins") {
        return [file("src/plugins/builtin/files/renderer/editor/cm-theme.ts")];
      }
      if (path === "src/other") {
    ...
    await materializePathQueryHits({
      list,
      paths: ["src/plugins/builtin/files/renderer/editor/cm-theme.ts"],
      root: ROOT,
    });
