...
vi.mock("@/lib/workspace/layout-persistence.ts", () => ({
...
} from "@/components/workspace/transfer/adapters.ts";
import {
...
} from "@/components/workspace/transfer/index.ts";
import {
  restoreEmbeddedTransferPanels,
  rewriteMissingComponentsToUnavailable,
} from "@/components/workspace/transfer/layout-rewrite.ts";
import {
...
function installPier(overrides: Record<string, unknown> = {}) {
  ...
  const finishDrag = vi.fn<
    (_transferId: string) => Promise<PanelTransferResult | null>
  >(async () => null);
