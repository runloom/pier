...
} from "@/panel-kits/mission-control/mission-control-ordered-layout.ts";
...
describe("responsive ordered Mission Control layout", () => {
  it.each([
    [0, 2],
    [188, 2],
    [388, 4],
  ...
  it("packs in strict Z order and uses the tallest item as row height", () => {
    const layout = deriveOrderedMissionControlLayout(
    ...
    expect(missionControlLayoutRows(layout)).toBe(7);
  });
  ...
  it("temporarily clamps width in a narrow container without mutating preference", () => {
    ...
    expect(
      deriveOrderedMissionControlLayout(widgets, { cols: 4 })[0]
    ).toMatchObject({ w: 4 });
    expect(widgets[0]).toMatchObject({ w: 8 });
    expect(
      deriveOrderedMissionControlLayout(widgets, { cols: 12 })[0]
    ).toMatchObject({ w: 8 });
  });

  it("uses host persistence bounds when a widget declaration is unavailable", () => {
    const [item] = deriveOrderedMissionControlLayout([entry("unknown", 1, 1)], {
      cols: 12,
    });
  ...
  it("moves only the ordered instance and keeps all entry data", () => {
    ...
    const moved = moveMissionControlEntry(widgets, "c", 1);
  ...
  it("does not let RGL displace siblings during repeated drag collisions", () => {
    ...
    let layout = deriveOrderedMissionControlLayout(
