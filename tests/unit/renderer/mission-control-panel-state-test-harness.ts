...
import { useMissionControlPanelState } from "@/panel-kits/mission-control/use-mission-control-panel-state.ts";
...
export function entry(
  ...
  w = 4,
  h = 3,
  extra: Partial<MissionControlPanelWidgetEntry> = {}
): MissionControlPanelWidgetEntry {
  return { h, id, w, ...extra };
}
...
export function widgetIds(snapshot: MissionControlPanelParams): string[] {
  return snapshot.widgets.map((widget) => widget.id);
}

export function widget(
  snapshot: MissionControlPanelParams,
  instanceId: string
): MissionControlPanelWidgetEntry {
...
export function renderState(
  ...
  const hook = renderHook(
    ({ api: currentApi, plugins: currentPlugins, value }: StateRenderProps) =>
      useMissionControlPanelState(value, currentApi, currentPlugins),
    {
  ...
  const snapshots = (targetApi: TestPanelParamsApi = api) =>
    targetApi.updateParameters.mock.calls.map(
      ([snapshot]) => snapshot as MissionControlPanelParams
    );
  return {
    ...hook,
    api,
