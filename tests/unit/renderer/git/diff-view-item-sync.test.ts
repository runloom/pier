...
describe("anchored item fallback", () => {
  ...
  it("flushes layout on content-only version bump without membership change", async () => {
    ...
    const next = [makeItem("section:a", 2, "a.ts")];
    const { handle, updateItem } = mockHandle({ items: previous });
    const instance = handle.getInstance();
    if (!instance) {
      throw new Error("expected a CodeView instance");
    }
    ...
    // 故意不传 flushLayout：正文变更也应自动 flush
    applyCodeViewItemsAnchored(handle, next, previous);
  ...
  it("uses updateItem for single estimate→loaded (avoids full-ledger setItems thrash)", async () => {
    ...
    const next = [
      {
        ...
        version: 2,
        fileDiff: {
          additionLines: ["+loaded\n"],
          cacheKey: "git-review-section:a:1:abc",
          deletionLines: ["-old\n"],
          hunks: [],
          isPartial: false,
          name: "a.ts",
    ...
    const { handle, setItems, updateItem } = mockHandle({ items: previous });
    const instance = handle.getInstance();
    if (!instance) {
      throw new Error("expected a CodeView instance");
    }
