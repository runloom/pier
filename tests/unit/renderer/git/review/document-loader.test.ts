...
describe("GitReviewDocumentLoader", () => {
  ...
  it("does not load documents until demand or selection is set", () => {
    ...
    // 选中即 demand：冷开/树点选不得等 window 回报才开始读，否则骨架永驻。
    loader.setProtectedEntryKey("entry:2000");
    expect(load).toHaveBeenCalledTimes(1);
    expect(load.mock.calls[0]?.[0]?.entryKey).toBe("entry:2000");
  });

  it("prioritizes protect-only selection without a Pierre window", () => {
    ...
    loader.setProtectedEntryKey("entry:7");
    expect(load.mock.calls.map(([item]) => item.entryKey)).toEqual(["entry:7"]);
    // 再次 set 同一 key 仍保持 demand（不因 early-return 丢掉）
    loader.setProtectedEntryKey("entry:7");
    expect(load).toHaveBeenCalledTimes(1);
  });
