...
import { useDiffViewCodeOptions } from "../../../../packages/ui/src/diff-view/use-code-options.ts";
import type {
  DiffViewCollapsedItemState,
  DiffViewRenderItemIdentity,
} from "../../../../packages/ui/src/diff-view/use-handle.ts";
import { useDiffViewHeaders } from "../../../../packages/ui/src/diff-view/use-headers.tsx";
...
it("局部 busy 变化不更换 hunk annotation renderer，并读取最新控制态", () => {
  ...
  const props = {
    ...
    collapseAllIntentRef: { current: null as boolean | null },
    ...
    metrics: {
      ...
      headerHeight: 32,
      lineHeight: 23,
      skeletonBodyHeight: 108,
      skeletonSlotHeight: 140,
    },
    onHunkAction: vi.fn(),
...
it("正文 annotation 不变时仍以实时 change capability 收起 Revert", () => {
  ...
  const hook = renderHook(() =>
    useDiffViewCodeOptions({
      ...
      codeViewRef: { current: null },
      collapseAllIntentRef: { current: null },
      diffStyle: "split",
      fileHoverCleanupsRef: { current: new Map<string, () => void>() },
      fileHoverHostsRef: { current: new Map<string, HTMLElement>() },
      inputStore,
