...
import { registerPluginWorkbenchWidget } from "@/lib/plugins/workbench-widget-registry.ts";
import { WorkbenchPanel } from "@/panel-kits/workbench/panel.tsx";
...
function renderPanel(
  ...
  return render(<WorkbenchPanel {...makeProps(params, updateParameters)} />);
}

describe("Workbench widget rendering", () => {
  it("renders plugin-disabled placeholder when plugin is disabled", () => {
    ...
    expect(
      screen.getByTestId("workbench-widget-pier.test.widget")
    ).toBeInTheDocument();
  });
  ...
  it("asks for confirmation before removing unknown widget inline", async () => {
    ...
    expect(
      screen
        .getByTestId("workbench-widget-unknown-remove")
        .closest('[data-slot="empty"]')
    ).toBeInTheDocument();
    expect(
      screen.queryByTestId("workbench-widget-menu-remove")
    ).not.toBeInTheDocument();
    expect(
      screen.getByTestId("workbench-widget-unknown-remove")
    ).toHaveAttribute("data-size", "default");
    fireEvent.click(screen.getByTestId("workbench-widget-unknown-remove"));
  ...
  it("asks for confirmation before removing a widget", async () => {
    ...
    fireEvent.click(await screen.findByTestId("workbench-widget-menu-remove"));
  ...
  it("catches widget errors and resets the boundary through refresh", async () => {
    ...
    function CrashingWidget(_props: WorkbenchWidgetComponentProps) {
      if (shouldCrash) {
        throw new Error("widget boom");
      }
      return <div>widget recovered</div>;
    }
    const crashRegistration: RendererWorkbenchWidgetRegistration = {
      component: CrashingWidget,
      icon: AlertTriangle,
      id: "pier.crash.widget",
    };
    registerPluginWorkbenchWidget(crashRegistration);
    setPluginRegistry([
      makePluginRegistryEntry({
        pluginId: "pier.crash",
  ...
  it("首渲后注册的插件 widget 从骨架屏占位切换为真实组件", () => {
    ...
    expect(screen.getByTestId("workbench-widget-loading")).toBeInTheDocument();
    expect(screen.queryByTestId("late-widget-body")).not.toBeInTheDocument();

    function LateWidget(_props: WorkbenchWidgetComponentProps) {
    ...
    expect(
      screen.queryByTestId("workbench-widget-loading")
    ).not.toBeInTheDocument();
  });
});
...
describe("物料库对话框", () => {
  it("点击添加入口打开物料库，选择物料后写入 v3 条目并关闭", async () => {
    ...
    fireEvent.click(
      screen.getByTestId("workbench-widget-picker-item-core.activity-overview")
    );
    ...
    expect(screen.queryByTestId("workbench-library")).not.toBeInTheDocument();
  });

  it("物料库支持搜索过滤", async () => {
    render(<WorkbenchPanel {...makeProps({ widgets: [] })} />);
    fireEvent.click(screen.getByTestId("workbench-add-widget"));
    const search = await screen.findByTestId("workbench-library-search");
    ...
    expect(
      screen.queryByTestId("workbench-widget-picker-item-core.system-resources")
    ).not.toBeInTheDocument();
  });
  ...
  it("物料库弹窗尺寸与设置弹窗对齐", async () => {
    render(<WorkbenchPanel {...makeProps({ widgets: [] })} />);
    fireEvent.click(screen.getByTestId("workbench-add-widget"));
    const dialog = await screen.findByTestId("workbench-library");
  ...
  it("物料库不暴露裸网格尺寸徽标", async () => {
    render(<WorkbenchPanel {...makeProps({ widgets: [] })} />);
    fireEvent.click(screen.getByTestId("workbench-add-widget"));
    await screen.findByTestId("workbench-library");
  ...
  it("单实例物料已添加后在库中禁点，多实例物料可重复添加", async () => {
    ...
    expect(
      screen.getByTestId("workbench-widget-picker-item-core.custom-card")
    ).not.toBeDisabled();
  });

  it("空态只显示添加物料入口，不再显示快速开始预设", () => {
    render(<WorkbenchPanel {...makeProps({ widgets: [] })} />);
    expect(screen.getByTestId("workbench-add-widget")).toBeInTheDocument();
    expect(
      screen.queryByTestId("workbench-preset-agent-workbench")
    ).not.toBeInTheDocument();
  });
});
...
describe("实例模型与组件菜单", () => {
  it("v3 条目（uuid 实例 + widgetId）按实例 id 渲染并带 data-widget-id", () => {
    ...
    expect(
      screen.getByTestId(
        "workbench-widget-1c8b6ee2-0000-0000-0000-000000000000"
      )
  ...
  it("multiInstance 物料的菜单提供复制，复制条目为新实例且保留 params", async () => {
    ...
    await openWidgetMenu();
    fireEvent.click(
      await screen.findByTestId("workbench-widget-menu-duplicate")
    );
  ...
  it("configurable 物料从菜单打开设置弹窗", async () => {
    ...
    await openWidgetMenu();
    fireEvent.click(
      await screen.findByTestId("workbench-widget-menu-settings")
    );
    expect(
      await screen.findByTestId("workbench-widget-settings-dialog")
    ).toHaveAttribute("role", "dialog");
    const dialog = screen.getByTestId("workbench-widget-settings-dialog");
