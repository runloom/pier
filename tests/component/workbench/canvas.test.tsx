...
import { installWorkbenchTestHarness, makeProps } from "./test-harness.ts";
...
describe("Workbench responsive grid integration", () => {
  it("derives fixed grid rhythm and Z-order coordinates from array order", () => {
    ...
    expect(
      ...
      { h: 1, i: "workbench-add", w: 2, x: 4, y: 4 },
    ]);
  });
  ...
  it("keeps repeated cross-slot drag previews under the ordered solver", () => {
    ...
    expect(
      screen.getByTestId("workbench-widget-core.activity-overview")
    ...
    expect(
      screen.getByTestId("workbench-widget-core.system-resources").parentElement
        ?.style.transform
    ).toContain("translate3d");
    expect(
      capturedGrid().layout?.map(({ h, i, w, x, y }) => ({ h, i, w, x, y }))
  ...
  it("shows live grid dimensions only while pointer resizing", () => {
    ...
    expect(screen.getByTestId("workbench-resize-size")).toHaveTextContent(
      "5 × 4"
    );
    ...
    expect(
      screen.queryByTestId("workbench-resize-size")
  ...
  it("deleting the middle widget immediately closes its visual gap", async () => {
    ...
    const systemCard = screen.getByTestId(
      "workbench-widget-core.system-resources"
    );
    fireEvent.pointerDown(
      within(systemCard).getByTestId("workbench-widget-menu-trigger"),
