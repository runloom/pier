...
import { FileEditorAdapter } from "@plugins/builtin/files/renderer/editor/adapter.tsx";
import { FileEditorController } from "@plugins/builtin/files/renderer/editor/controller.ts";
import type * as MarkdownParserModule from "@plugins/builtin/files/renderer/markdown/parser.ts";
import type * as MarkdownRuntimeModule from "@plugins/builtin/files/renderer/markdown/runtime.ts";
import {
  createFileFilePanelInstanceId,
  fileFilePanelIdentityKey,
} from "@plugins/builtin/files/renderer/panel/id.ts";
import { createFilePanel as createFilePanelWithController } from "@plugins/builtin/files/renderer/panel/index.tsx";
import {
...
import { FileImagePreview } from "@plugins/builtin/files/renderer/preview/image.tsx";
import { saveAsJournalForDocument } from "@plugins/builtin/files/renderer/save/as-journal.ts";
import {
  clearFileTreeSidebarCache,
  openFilesTreeSearch,
} from "@plugins/builtin/files/renderer/tree/registry.ts";
import { FileTreeSidebar } from "@plugins/builtin/files/renderer/tree/sidebar.tsx";
import { clearFilesTreeStore } from "@plugins/builtin/files/renderer/tree/store.ts";
import { FilesWatchHub } from "@plugins/builtin/files/renderer/watch-hub.ts";
import {
...
vi.mock("@plugins/builtin/files/renderer/markdown/runtime.ts", async () => {
  const actual = await vi.importActual<typeof MarkdownRuntimeModule>(
    "@plugins/builtin/files/renderer/markdown/runtime.ts"
  );
  const parser = await vi.importActual<typeof MarkdownParserModule>(
    "@plugins/builtin/files/renderer/markdown/parser.ts"
  );
...
type PathQueryListener = (
  event: import("@shared/contracts/file/query.ts").FileQueryEvent
) => void;

function createPathQueryMock() {
  const listeners = new Set<PathQueryListener>();
  const starts: import("@shared/contracts/file/query.ts").FilePathQueryStart[] =
    [];
  let nextId = 0;
  return {
    starts,
    emit(event: import("@shared/contracts/file/query.ts").FileQueryEvent) {
    ...
    queryPaths(
      request: Omit<
        import("@shared/contracts/file/query.ts").FilePathQueryStart,
        "queryId"
      > & { queryId?: string }
    ) {
      nextId += 1;
      ...
      starts.push({
        ...request,
        queryId,
      } as import("@shared/contracts/file/query.ts").FilePathQueryStart);
      return {
        cancel: vi.fn(),
        queryId,
...
describe("Files file-panel", () => {
  ...
  it("localizes file-panel chrome through plugin i18n messages", async () => {
    const translations = new Map<string, string>([
      ...
      ["filePanel.view.switchToPreview", "[切换到预览]"],
      ["filePanel.view.switchToSource", "[切换到源码]"],
    ]);
    ...
    expect(screen.getByRole("button", { name: "[切换到预览]" })).toBeVisible();
      ...
      async (_requestOrRoot, options) => {
        ...
        if (path === "src/plugins/builtin/files/renderer") {
          return [
            {
              kind: "file",
              path: "src/plugins/builtin/files/renderer/editor/cm-theme.ts",
              root: PROJECT_ROOT,
            },
          ];
        }
  ...
  it("renders the panel owner's language status immediately after the language badge", async () => {
    ...
    expect(languageStatus).not.toHaveAttribute("tabindex");
  ...
  it("hides the Markdown mode toggle for non-Markdown documents", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Switch to source" })
    ).toBeNull();
  });
  ...
  it("installs a high-priority CodeMirror DOM handler before basicSetup can open the default search panel", async () => {
    const editorSource = await readFile(
      join(
        process.cwd(),
        "src/plugins/builtin/files/renderer/editor/view-extensions.ts"
      ),
      "utf8"
    );
  ...
  it("keeps CodeMirror gutters opaque and sticky so horizontal scroll cannot bleed into line numbers", async () => {
    ...
    const editorSource = await readFile(
      join(process.cwd(), "src/shared/source-editor/theme.ts"),
      "utf8"
    );
