...
import { createFilesQuickOpenAction } from "@plugins/builtin/files/renderer/search/quick-open.ts";
import {
  __resetFilesPathMruForTests,
  listFilesPathMru,
} from "@plugins/builtin/files/renderer/search/quick-open-mru.ts";
import type {
  FilePathQueryStart,
  FileQueryEvent,
...
describe("files quick open", () => {
  ...
  it("opens an async quick pick and streams path query results into updateQuickPick", async () => {
    ...
    expect(lastPatch.items?.[0]).toMatchObject({
      description: "src/plugins/builtin/files/renderer/editor/cm-theme.ts",
      label: "cm-theme.ts",
    });
