...
- 持久化条目 `{id, widgetId?, params?, x, y, w, h}`：`id` 是实例 id（多实例物料为 uuid），`widgetId` 是物料 id（v1 条目缺席时回退 `id`，零迁移）；`params` 是物料私有配置，宿主视为黑盒 JSON，校验责任在物料边界（自带 zod schema，非法值逐条降级）。面板级 `locked` 锁定布局。
...
- 声明元数据：`category`（物料库分类）、`searchTerms`（搜索命中面）、`multiInstance`（可复制/重复添加）、`configurable`（注册时须提供 `settingsComponent`，渲染进宿主设置 Sheet）、`refreshable`（菜单显示"刷新"）；注册可带 `previewComponent`（物料库预览卡，样例数据静态渲染）。
- 添加入口是物料库对话框（分类 + 搜索 + 预览），不是下拉菜单；空态和底部添加入口只打开物料库，不提供快速开始预设。
- 指标目录（`src/renderer/lib/mission-control/metric-registry.ts`）：core/插件用代码贡献指标（instant/series/grouped × 格式），"自定义卡片"物料把区块（kpi/gauge/trend/ranking）绑定到指标做用户级组装；不做查询语言、不做自由画布。
...
- 网格几何：格子像素恒定（`CELL_WIDTH = 88`），面板宽度只决定可用列数 k；持久化的 `widgets`
  数组始终是 12 列基准布局。12 列默认尊重用户拖拽/调整尺寸后的持久化布局，窄容器用
  `deriveLayout` 派生显示布局且不把 k 列坐标直接写回。自动整理是显式动作，走
  `deriveOptimalAutoLayout` 保序行断点动态规划，目标是高利用率、阅读顺序稳定、少空洞、
  避免可避免的普通孤儿卡，并让高优先级物料优先变宽。widget 内容响应一律用 container query
  （卡片 `CardContent` 已开 `@container`），不要依赖 `size` prop 换算像素。注意 containment
  会让 `position: fixed` 后代以卡片内容区为包含块——浮层一律走 portal（Radix 组件默认如此）。
