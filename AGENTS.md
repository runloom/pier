...
- 迁移期与老 `agent-session` broadcast 双写，见 `local://finalize-double-source-plan.md`
- 模块内不 import `services/agents/`（agent 只是 activity 的一种 kind，边界单向）

### 项目实体 `src/main/state/project-store.ts` + `src/shared/contracts/project.ts`

- `Project = { id: uuid, rootPath, name, updatedAt }` 持久化到 `${userData}/project-state.json`
- `upsertProjectFromPath()` in-flight 去重防并发；name 派生 package.json > deno.json > Cargo.toml [package] > basename
- `PanelContext.projectId` + `projectRootPath` 是渐进迁移目标，与老 `projectRoot` 并存直到 60+ callsite 迁完
