...
- **产品词全产品统一。** 当前约定：智能体（不要混用 Agent/agent）、工作树（中文界面不要写 worktree）、Canvas 发现面「物料」（仓库 `.pier/canvases/canvas-kit`，后续官网文档；不要做进设置）、需要你处理（中文不要直出 Needs you）、git（小写；不要写成 Git / GIT；GitHub 等专有名除外）。界面语言与根 README 的语言集合均为 `SUPPORTED_LOCALES`（`zh-CN` / `en` / `ja` / `ko`）。
