...
첫 작업은 확인된 네이티브 대화형 실행 인수로 전달합니다. 지원하지 않는 명령이나 확인되지 않은 버전에서는 향상된 입력창에 초안으로 저장하고 수동 전송을 기다립니다. `inputDisposition`은 `native-launch`, `draft`, `unconfirmed` 중 하나입니다. `creationStatus: unconfirmed`이면 다시 실행하기 전에 패널을 확인하세요. 네이티브 승인은 터미널에서 직접 완료합니다.

전송하지 않은 초안은 첨부 파일과 편집 형식을 포함하여 저장되며 앱 재시작 후 복원됩니다. 전송이 중단되면 확인이 필요한 상태로 남고 자동 재전송하지 않습니다. `agents turn`은 마지막 줄바꿈까지 보존하고 전송 전에 체크포인트를 저장합니다. 수락은 전송 확인만을 뜻합니다.

`agents interrupt`는 네이티브 Ctrl+C를 보냅니다. `agents terminate`는 TERM을 보내고 2 초 후 KILL로 전환하며 호스트가 시작한 프로세스의 종료를 최대 10 초 기다립니다. 시간 초과는 오류입니다. 중지 후에도 앱을 종료하기 전까지 패널과 출력을 읽고 복사할 수 있습니다. 패널을 닫으려면 `terminal close`를 사용하세요. 터미널에서 분리된 서비스의 정리나 작업 성공을 보장하지 않습니다.
