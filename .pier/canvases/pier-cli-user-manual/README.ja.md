...
最初のタスクは、確認済みのネイティブ対話型起動引数で渡します。未対応のコマンドや未確認のバージョンでは、拡張入力欄に下書きとして保存し、手動送信を待ちます。`inputDisposition` は `native-launch`、`draft`、`unconfirmed` のいずれかです。`creationStatus: unconfirmed` の場合は、重複起動する前にパネルを確認してください。ネイティブの承認はターミナルで手動で行います。

未送信の下書きは添付ファイルと編集形式を含めて保存され、再起動後に復元されます。送信が中断した場合は要確認となり、自動再送しません。`agents turn` は末尾の改行も保持し、送信前にチェックポイントを保存します。受理は転送の確認に限ります。

`agents interrupt` はネイティブ Ctrl+C を送ります。`agents terminate` は TERM、2 秒後に KILL を送り、宿主が起動したプロセスの終了を最大 10 秒待ちます。タイムアウトはエラーです。停止後もアプリ終了まではパネルと出力を閲覧・コピーできます。パネルを閉じるには `terminal close` を使います。ターミナルから独立したサービスの回収や、タスクの成功を保証するものではありません。
