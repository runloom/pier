...
export function HostScreen(props: {
  ...
  const waitingSessions = props.sessions.filter(
    (session) => session.status === "waiting"
  );
  const waiting = waitingSessions.length;
  const onlyWaiting = waitingSessions[0];
  const worktrees = [
    ...new Set(
      props.sessions
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-7 overflow-y-auto overscroll-contain px-4 pt-5 pb-8 [scrollbar-width:thin]">
          {worktrees.length === 0 ? null : (
            <section aria-label="工作树变更" className="flex flex-col gap-2">
              <h2 className="px-0.5 font-semibold text-[15px] leading-5">
                工作树变更
              </h2>
          ...
          <section aria-label="会话" className="flex flex-col gap-4">
            ...
            >
              {props.sessions.map((session) => (
                <button
                  aria-label={`${session.title}，${session.agent ?? "终端"}，${SESSION_STATE_LABEL[session.status]}，打开会话`}
                  className={cx(
                    "group min-h-11 min-w-0 rounded-xl text-left",
                    TOUCH_PRESS
                  )}
                  key={session.id}
