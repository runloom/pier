...
- **运行任务带上准备命令。** 有 mise 配置时可以信任并安装工具链；有 package.json、Composer、uv 或 Poetry 时可以安装依赖。本机没有对应程序时，这些命令不会出现。

### Changed

- **运行任务不再列出 Zed 的任务。** 已经开着的终端还在，只是不再带这条任务记录。

## [0.1.44] - 2026-09-22

### Added

- **审查冲突一次只看当前文件。** 合并更改选中一项；带标记的冲突可以在这里接受并暂存。工作区文件不在、两边又能读成文本时，正文是当前侧对传入侧的差异。文件太大、编码不对或读不出来，且文件还在时，主按钮是打开文件。

### Changed

- **审查先显示改动块。** 点开折叠的未修改行时再读取全文。侧栏已经换过时，正文等列表刷新，不再拿过期索引重试。
...
- **项目记忆。** 设置 → 项目里可为仓库登记受管智能体配置（例如 MCP），下次打开项目即可用。
- **同网移动端接入。** 设置里可配对手机，在同一局域网查看会话、文件和变更。
- **HTML / SVG 预览。** 文件面板能预览 HTML 和 SVG，缩放操作与图片预览一致。
...
- **应用图标改为单一矢量母版。** 各尺寸从同一 SVG 降采样，去掉 Dock 专用位图。
- **系统字体优先。** 不再打包 HarmonyOS Sans SC，安装包减小约 31MB。
- **终端优先快捷键。** 焦点在终端时，查找等快捷键优先交给终端；需要命令面板时用 ⌘⇧P。
...
- **宿主重启后接回智能体会话。** 重启 Pier 后仍显示并接回未结束的智能体会话。
- **Git Review 折叠侧栏查找。** 侧栏收起时点查找会先展开再打开搜索。
